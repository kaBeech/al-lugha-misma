...
const challenge_key = async (ctx: Context) => {
  ...
  const challenge_key_result = await client.queryObject(
    `SELECT REFERENCE_WORD_ENGLISH, TRANSLITERATED_WORD, LANGUAGES.LANGUAGE FROM LANGUAGES JOIN TRANSLITERATED_WORDS ON (LANGUAGES.ID=TRANSLITERATED_WORDS.LANGUAGE) JOIN REFERENCE_WORDS_ENGLISH ON (REFERENCE_WORD_ENGLISH=REFERENCE_WORDS_ENGLISH.ID) JOIN CHALLENGES ON (REFERENCE_WORDS_ENGLISH.CHALLENGE=CHALLENGES.ID) WHERE CHALLENGES.NAME = ${ctx.request.body.arguments.challengeName} AND LANGUAGES.LANGUAGE IN (${languagesFormatted})`,
  );
